...
function getlastlog(){
    ...
    $.ajax({
        ...
        $.each(data.data.items,function(k,v){
            ...
            if(v.update_timestemp > 240){
                $('#device-'+v.device_id).addClass('lostconnect');
                $('#device-'+v.device_id+' .icon').html('<i class="far fa-exclamation-triangle"></i>');

            }else if(v.device_alert){
                $('#device-'+v.device_id).addClass('alret');
                $('#device-'+v.device_id+' .icon').html('<i class="far fa-exclamation-circle"></i>');
            }else{
                $('#device-'+v.device_id).addClass('active');
                $('#device-'+v.device_id+' .icon').html('<i class="far fa-spinner fa-pulse"></i>');
            }
